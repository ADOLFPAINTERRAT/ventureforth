...
export default function PhotoMemories({ map, photos, onMove, onResize, onDelete }: Props) {
  ...
  // Deselect when tapping empty map space (backdrop)
  useEffect(() => {
    const onBackdropPointerDown = (e: PointerEvent) => {
      const target = e.target as HTMLElement | null;
      if (target && !target.closest("[data-photo]")) {
        setSelectedId(null);
      }
    };
    document.addEventListener("pointerdown", onBackdropPointerDown);
    return () => document.removeEventListener("pointerdown", onBackdropPointerDown);
  }, []);
        ...
        {photos.map((photo) => {
          ...
          return (
            <div
              key={photo.id}
              data-photo
              className={selectedId === photo.id ? "ring-2 ring-[oklch(0.62_0.21_27)]" : ""}
              style={{ left: x, top: y, width: px, height: px }}
